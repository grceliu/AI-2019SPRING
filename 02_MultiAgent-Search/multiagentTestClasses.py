    ...
    def getAction(self, state):
        ...
        for i in range(len(optimalActions)):
            if studentAction[0] in optimalActions[i][0]:
                ...
                #print "Okie", studentAction[0], optimalActions[i][0]
            else:
                #print "NoNo", studentAction[0], optimalActions[i][0]
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for passed, points, value, minimum, thresholds, name in results:
            ...
            if not passed:
                ...
                self.addMessage("%s %s (%s of %s points)" % (value, name, points, len(thresholds)*5))
            ...
            if minimum != None:
                ...
                for idx, threshold in enumerate(thresholds):
                    self.addMessage("    >= %s:  %s points" % (threshold, (idx+1)*5))
