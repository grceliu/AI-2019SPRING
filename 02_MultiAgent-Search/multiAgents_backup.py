...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def minimax(self, state, current):
        ...
        score = [self.value(state, current + 1)[1] for state in successors_states]
    ...
    def value(self, state, current):
        if state.isWin() or state.isLose() or current == self.depth * state.getNumAgents():#reach the limit
            return (None, self.evaluationFunction(state))
        elif current % state.getNumAgents() != 0: #ghost
            return self.min_node(state, current)
        else: #pacman
            return self.max_node(state, current)

    def max_node(self, state, current):
        actions = state.getLegalActions(0)
        successors_states = [state.generateSuccessor(0, i) for i in actions]
        score = [self.value(state, current + 1)[1] for state in successors_states]
        return (actions[score.index(max(score))], max(score))

    def min_node(self, state, current):
        agent_id = current % state.getNumAgents()
        actions = state.getLegalActions(agent_id)
        successors_states = [state.generateSuccessor(agent_id, i) for i in actions]
        score = [self.value(state, current + 1)[1] for state in successors_states]
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        """
        ans = self.value(gameState,0, -float("inf"), float("inf"))
        return ans[0]

    def value(self, state, current, alpha, beta):
        if state.isWin() or state.isLose() or current == self.depth * state.getNumAgents():#reach the limit
            return (None, self.evaluationFunction(state))
        ...
        if agent_id == 0:
            ...
            for action in actions:
                ...
                result = self.value(successor, current + 1, alpha, beta)
                if result[1] > v[1]:
                    v = (action, result[1])
                if v[1] > beta:
                    return v
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def value(self, state, current):
        if state.isWin() or state.isLose() or current == self.depth * state.getNumAgents():#reach the limit
            ...
            return self.expectimax(state, current)
